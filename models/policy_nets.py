...
class TransformerActorNetwork(MIMO_Transformer):
    ...
    def __init__(
        ...
        self._is_goal_conditioned = False
        #print("goal_shapes:", goal_shapes)
        #print("dim", len(goal_shapes))
        if goal_shapes is not None and len(goal_shapes) > 0:
            assert isinstance(goal_shapes, OrderedDict)
            self._is_goal_conditioned = True
            self.goal_shapes = OrderedDict(goal_shapes)
...
class TransformerActorChunkingNetwork(TransformerActorNetwork):
    """
    Transformer policy that predicts a chunk of future actions from the final timestep
    in observation sequence.
    """

    def __init__(self, action_chunk_size=32, **kwargs):
        self.action_chunk_size = action_chunk_size
        super().__init__(**kwargs)

    def _get_output_shapes(self):
        """
        Output shape for predicting multiple future actions:
        - Each sample predicts `action_chunk_size` future actions
        - Each action has dim = ac_dim
        """
        return OrderedDict(
            action=(self.action_chunk_size, self.ac_dim)
        )

    def forward_train(self, obs_dict, actions=None, goal_dict=None, supervised=False):
        """
        Forward pass: output full action chunk for the future.

        If supervised=True, combine the first timesteps (for chunk index 0 across time)
        with the last transformer's timestep chunks along the time dimension, similar to
        the GMM chunking supervised behavior.
        """

        # Goal conditioning support (same as parent)
        if self._is_goal_conditioned:
            assert goal_dict is not None
            mod = list(obs_dict.keys())[0]
        ...
        # Run MIMO transformer directly to get full outputs dict
        outputs = MIMO_Transformer.forward(self, **forward_kwargs)

        # outputs["action"] expected shape: [B, T, chunk, ac_dim]
        if supervised:
            # Combine first-timestep chunk-0 across time with the last timestep's chunks
            first = outputs["action"][:, :, 0, :]   # [B, T, ac_dim]
            last = outputs["action"][:, -1, :, :]   # [B, chunk, ac_dim]
            action_preds = torch.cat((first, last), dim=1)  # [B, T+chunk, ac_dim]
        else:
            # Only use LAST timestep's encoded features → future predictions
            action_preds = outputs["action"][:, -1, :, :]  # [B, chunk, ac_dim]

        # Squash to [-1,1]
        action_preds = torch.tanh(action_preds)

        return action_preds   # [B, sequence_len, ac_dim]
    def forward(self, obs_dict, actions=None, goal_dict=None, supervised=False):
        """
        Return action chunk tensor. `supervised` is forwarded to forward_train.
        """
        out = self.forward_train(obs_dict=obs_dict, actions=actions, goal_dict=goal_dict, supervised=supervised)
        return out

    def _to_string(self):
        return f"action_dim={self.ac_dim}, chunk_size={self.action_chunk_size}"
...
class TransformerGMMActorNetwork(TransformerActorNetwork):
    ...
    def forward_train(self, obs_dict, actions=None, goal_dict=None, low_noise_eval=None):
        ...
        if self.use_tanh:
            # Wrap distribution with Tanh
            dists = TanhWrappedDistribution(base_dist=dists, scale=1.)
        print("Distribution at timestep {}: {}".format(1, dists))
...
class TransformerGMMActorNetworkActionChunking(TransformerGMMActorNetwork):
    def __init__(self, action_chunk_size = 32, **kwargs):
        self.action_chunk_size = action_chunk_size
        print("kwargs: ", {**kwargs})
        super().__init__(**kwargs)
    ...
    def _get_output_shapes(self):
        """
        Output GMM parameters for each future prediction timestep.
        Each timestep gets its own set of GMM parameters.
        """
        
        return OrderedDict(
            mean=(self.action_chunk_size, self.num_modes, self.ac_dim),    # [chunk_size, modes, action_dim]
            scale=(self.action_chunk_size, self.num_modes, self.ac_dim),   # [chunk_size, modes, action_dim]
            logits=(self.action_chunk_size, self.num_modes),               # [chunk_size, modes]
        )

    def forward_train(self, obs_dict, actions=None, goal_dict=None, low_noise_eval=None, supervised=False):
        """
        Return GMM distributions for each future prediction timestep.
        """
        #print("obs_dict keys:", obs_dict["eef_pos"].shape)
        ...
        if supervised:
            # Combine first and last time steps along the T dimension
            #print("First: ,", outputs["mean"][:, :, 0, :, :].shape, outputs["scale"][:, :, 0, :, :].shape, outputs["logits"][:, :, 0, :].shape)
            #print("Last: ,", outputs["mean"][:, -1, :, :, :].shape, outputs["scale"][:, -1, :, :, :].shape, outputs["logits"][:, -1, :, :].shape)
            means  = torch.cat(
                (outputs["mean"][:, :, 0, :, :],   # keep 5D shape
                outputs["mean"][:, -1, :, :, :]),
                dim=1
            )
            scales = torch.cat(
            ...
            means  = outputs["mean"][:, -1, :, :, :]
            scales = outputs["scale"][:, -1, :, :, :]
            logits = outputs["logits"][:, -1, :, :]
        #print("++++++++++++++++++++++++++++++++++++++++++++++++,", means.shape, scales.shape, logits.shape)
        # Apply tanh to means if not using tanh-GMM
        """if not self.use_tanh:
            means = torch.tanh(means)
"""
        # Handle low noise evaluation
        if low_noise_eval is None:
            low_noise_eval = self.low_noise_eval
        if low_noise_eval and (not self.training):
            scales = torch.ones_like(means) * 1e-4
        ...
        # Create separate GMM distribution for each future timestep
        #distributions = []
        #for t in range(self.action_chunk_size):
        """component_distribution = D.Normal(loc=means[:, t, :, :], scale=scales[:, t, :, :])
        component_distribution = D.Independent(component_distribution, 1)
        mixture_distribution = D.Categorical(logits=logits[:, t, :])"""
        component_distribution = D.Normal(loc=means[:, :, :, :], scale=scales[:, :, :, :])
        component_distribution = D.Independent(component_distribution, 1)
        mixture_distribution = D.Categorical(logits=logits[:, :, :])
        dist = D.MixtureSameFamily(
        ...
        #print("Distribution at timestep {}: {}".format(1, dist))
        #distributions.append(dist)

        return dist  # List of distributions, one per future timestep

    def forward(self, obs_dict, actions=None, goal_dict=None):
        """
        Sample actions from each future timestep distribution.
        """
        #print("Forwarding through TransformerGMMActorNetworkActionChunking")
        #print("obs_dict keys:", obs_dict)
        dists = self.forward_train(obs_dict=obs_dict, actions=actions, goal_dict=goal_dict)
        
        # Sample from each distribution
        """action_chunks = []
        for dist in dists:
            action_chunks.append(dist.sample())  # [B, ac_dim]"""

        return dists.sample() #action_chunks
